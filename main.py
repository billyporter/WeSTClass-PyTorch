...
def main():
    ...
    if args.data == "generate":
        ...
        if args.pretrain == True:
            if args.model in ("rnn", "cnn"):
                ...
                seed_docs = np.load('seed_docs_{}.npy'.format(args.model), allow_pickle=True).item()
                seed_label = np.load('seed_labels_{}.npy'.format(args.model))
        if args.selftrain == True or args.evaluate == True:
            if args.model in ("rnn", "cnn"):
                ...
                x = np.load('real_docs_{}.npy'.format(args.model), allow_pickle=True).item()
                y = np.load('real_labels_{}.npy'.format(args.model))
    ...
    if args.pretrain == True:
        ...
        wstc.pretrain(train_loader, pretrain_epochs)
    ...
    if args.selftrain == True:
        ...
        wstc.self_train(self_train_loader, x, y, maxiter=maxiter, update_interval=update_interval)
